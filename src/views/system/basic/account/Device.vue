...
<script>
import { resourceConfigCategoryQuery, resourceDeviceDataQueryDeviceList } from '@/api/resource';
...
export default {
  ...
  methods: {
    ...
    loadNode(node, resolve) {
      ...
      if (node.level === 1) {
        this.$$ALoading.show();
        resourceConfigCategoryQuery({ parentId: node.data.id, type: 'DEVICE' })
          .then(res => {
      ...
      if (node.level === 2) {
        this.$$ALoading.show();
        resourceDeviceDataQueryDeviceList({ categoryId: node.data.id })
          .then(res => {
            const { code, object } = res;
            if (code === 200) {
              resolve(object);
