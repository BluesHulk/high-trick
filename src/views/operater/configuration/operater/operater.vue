...
import { operationStaffList, operationStaffListdel } from '@/api/order';
export default {
  name: 'opa-project',
  components: {
    APagination,
  ...
  methods: {
    ...
    delItem() {
      // 运维人员删除
      operationStaffListdel(this.rowData.id)
        .then(res => {
    ...
    getList(name) {
      ...
      operationStaffList(query, data).then(res => {
