    ...
    <div class="pr-30 pl-30 pt-30">
      <div class="resource-formStyle">
        <el-form ref="form" :model="formData" :rules="formRule">
          <el-row :gutter="24">
            <el-col :span="6">
              <el-form-item label="其他资产名称" prop="name">
                <el-input
                  v-model="formData.name"
                  placeholder="请输入其他资产名称"
            ...
            <el-col :span="6">
              <el-form-item label="其他资产编号" prop="code">
                <el-input v-model="code" placeholder="请输入其他资产编号" disabled></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="其他资产位置" prop="positionDesc">
                <el-input
                  v-model="formData.positionDesc"
                  placeholder="请输入其他资产位置"
                  :disabled="typeFrom == 'views'"
                  @input="onInput()"
                ></el-input>
              </el-form-item>
            ...
            <el-col :span="6">
              <el-form-item label="厂家信息" prop="factoryDesc">
                <el-input v-model="formData.factoryDesc" :disabled="typeFrom == 'views'" @input="onInput()"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
...
import { assetAdd, assetUpdate, getCategoryQuery, getSpaceData, queryByCategoryId } from '@/api/config';
...
export default {
  ...
  data() {
    return {
      ...
      formRule: {
        ...
        factoryDesc: [{ max: 60, message: '最多输入60个字符', trigger: 'change' }],
        longitude: [
          {
  ...
  methods: {
    ...
    showBack() {
      ...
      this.$set(this.formData, 'storageTime', this.currentData.storageTime);
      ...
      this.formData.factoryDesc = this.currentData.factoryDesc;
      if (this.currentData.spaceData) {
        this.formData.spaceId = this.currentData.spaceData.id;
      } else {
        this.formData.spaceId = '';
      }
      this.editCategoryList = [this.currentData.category];
      this.formData.categoryId = this.currentData.categoryId;
      this.groupList = [
        { name: '关联设备', related: 'XASSET', categoryId: this.currentData.category.id },
        { name: '关联其他资产', related: 'XASSET', categoryId: this.currentData.category.id },
        { name: '关联项目', related: 'XASSET', categoryId: this.currentData.category.id }
      ];
    },
    // 类别树
    deviceCategory(val) {
    ...
    getQueryById(node, resolve) {
      if (node.data) {
        getCategoryQuery({ type: 'XASSET', parentId: node.data.id }).then(res => {
          if (res.code == 200) {
            this.categoryList = res.object.filter(v => v.enable);
            const { level } = node;
            const nodes = this.categoryList.map(item => ({
              name: item.name,
              id: item.id,
              leaf: level < 2
            }));
    ...
    changeCategory(val) {
      this.attributes = [];
      // 根据类型获取动态分组属性
      queryByCategoryId({ type: 'XASSET', categoryId: val[val.length - 1] }).then(res => {
        if (res.code == 200) {
          const groupList = [
            { name: '关联设备', related: 'XASSET', categoryId: this.currentData.categoryId },
            { name: '关联其他资产', related: 'XASSET', categoryId: this.currentData.categoryId },
            { name: '关联项目', related: 'XASSET', categoryId: this.currentData.categoryId }
          ];
    ...
    save() {
      if (this.typeFrom == 'add') {
        this.$refs['form'].validate(valid => {
          if (valid) {
            ...
            assetAdd({
              ...
              if (res.code == 200) {
                this.$message.success('其他资产添加成功');
                this.$emit('submit-add');
              } else {
                this.$message.error(res.message || '其他资产添加失败');
              }
            });
          }
        });
        ...
        assetUpdate({
          ...
          if (res.code == 200) {
            this.$message.success('其他资产编辑成功');
            this.$emit('submit-add');
          } else {
            this.$message.error(res.message || '其他资产编辑失败');
          }
        });
      }
    },
