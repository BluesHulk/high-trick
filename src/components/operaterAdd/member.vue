...
<script>
import { inspectStaffTree, inspectStaffTreePeople, inspectStaffListAdd } from '@/api/order';
...
export default {
  ...
  methods: {
    ...
    addHaveCount(flag) {
      ...
      for (let i = 0; i < this.childrenCheckBox.length; i++) {
        ...
        for (let j = 0; j < this.checkList.length; j++) {
          ...
          if (ckItem === item) {
            this.checkList[j].inspectStaff = flag;
          }
        }
      }
    },
    addNorAccount(flag) {
      ...
      for (let m = 0; m < this.normalList.length; m++) {
        ...
        for (let n = 0; n < this.checkList.length; n++) {
          ...
          if (ckItem === item) {
            this.checkList[n].inspectStaff = flag;
          }
        }
      }
    },
    ...
    async nodeClick(data) {
      ...
      this.treeId = data;
      if (Object.keys(data).includes('inspectStaff')) return;
      this.$$ALoading.show();
      const result = await inspectStaffTreePeople({
        orgId: data.id
      });
      this.$$ALoading.close();
